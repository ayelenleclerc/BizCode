...
function portalT(key: string, opts?: { count?: number }): string {
...
describe('PortalFacturasPage', () => {
  ...
  it('enables pay online when mpPaymentLink is present', async () => {
    listFacturas.mockResolvedValue({
      facturas: [
        {
          id: 2,
    ...
    const payLink = screen.getByTestId('portal-factura-pay-2')
    expect(payLink).toHaveAttribute('href', 'https://mp.test/pay')
  })
})
