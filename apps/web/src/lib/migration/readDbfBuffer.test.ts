...
describe('assertValidDbfUploadBuffer', () => {
  ...
  it('accepts DBF files generated by dbffile', async () => {
    const fs = await import('node:fs/promises')
    const os = await import('node:os')
    const path = await import('node:path')
    const { DBFFile } = await import('dbffile')

    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'bizcode-dbf-validate-'))
    ...
    try {
      ...
      expect(() => assertValidDbfUploadBuffer(buffer)).not.toThrow()
    } finally {
