...
function buildMessage(type: NotificationType, payload: NotificationPayload): MessageTemplate {
  ...
  switch (type) {
    ...
    case 'tiendanube_order_imported': {
      const order = payload.resource ? ` ${payload.resource}` : ''
      return {
        subject: `[BizCode] Nueva venta en Tiendanube${order}`,
        text:
          payload.detail ??
          `Se importó una orden de Tiendanube${order}${payload.pedidoId != null ? ` como pedido #${payload.pedidoId}` : ''}.`,
      }
    }
    case 'tiendanube_cuit_required': {
      return {
        subject: `[BizCode] CUIT requerido para facturar orden Tiendanube`,
        text:
          payload.detail ??
          `Completar CUIT del cliente antes de emitir factura A${payload.pedidoId != null ? ` (pedido #${payload.pedidoId})` : ''}.`,
      }
    }
    case 'tiendanube_order_cancelled_invoiced': {
      return {
        subject: `[BizCode] URGENTE: Orden Tiendanube cancelada ya facturada`,
        text:
          payload.detail ??
