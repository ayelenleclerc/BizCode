...
import { registerSellerPushToken, unregisterSellerPushToken } from '../push/registerPush'
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const refreshSession = useCallback(async () => {
    ...
    if (nextStatus === 'authenticated') {
      try {
        pushTokenRef.current = await registerSellerPushToken()
      } catch {
  ...
  const logout = useCallback(async () => {
    await unregisterSellerPushToken(pushTokenRef.current)
