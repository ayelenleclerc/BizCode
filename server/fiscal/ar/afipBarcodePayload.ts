...
export type AfipBarcodeBuildInput = {
  cuitEmisor: string
  tipo: string
  prefijo: string
  cae: string
 ...
 */
export function buildAfipBarcodePayload(input: AfipBarcodeBuildInput): string {
  const cuit = normalizeCuitDigits(input.cuitEmisor).padStart(11, '0').slice(-11)
  const tipo = String(afipTipoComprobanteFromLetter(input.tipo)).padStart(2, '0').slice(-2)
