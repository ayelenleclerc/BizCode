...
import { mockRequestCae, mockRequestTa, type ArcaTaResult } from './arcaWsfeMock'
...
const taCache = new Map<number, ArcaTaResult>()
...
export class ArcaService {
  ...
  async getTa(tenantId: number): Promise<ServiceResult<ArcaTaResult>> {
  ...
  async requestCaeForFactura(
    ...
    try {
      ...
      await this.markPendingCae(facturaId)
      return { ok: false, status: 502, error: 'ARCA_CAE_REQUEST_FAILED' }
    }
  }
