...
import { ARCA_FE_QR_BASE_URL, type ArcaQrJsonPayload } from './arcaQrPayload'
import {
  arcaTipoComprobanteToLetter,
  formatPrefijoFromPtoVta,
  normalizeCuitDigits,
} from './arcaComprobanteCodes'

/** @en Result of local AFIP/ARCA QR decode (no portal API call). */
export type ArcaQrDecodeResult = {
  payload: ArcaQrJsonPayload
  source: 'url_param' | 'image_scan'
}

const ARCA_QR_URL_RE =
 ...
 */
export function decodeArcaQrParam(encoded: string): ArcaQrJsonPayload {
  const normalized = encoded.replace(/-/g, '+').replace(/_/g, '/')
  const pad = normalized.length % 4 === 0 ? '' : '='.repeat(4 - (normalized.length % 4))
  const json = Buffer.from(`${normalized}${pad}`, 'base64').toString('utf8')
  const parsed = JSON.parse(json) as ArcaQrJsonPayload
  if (parsed.ver !== 1) {
    throw new Error('Unsupported ARCA QR version')
  }
  return parsed
}
 ...
 */
export function parseArcaQrContent(content: string): ArcaQrJsonPayload | null {
  const trimmed = content.trim()
  if (trimmed.length === 0) return null

  const urlMatch = trimmed.match(ARCA_QR_URL_RE)
  if (urlMatch?.[1]) {
    try {
      return decodeArcaQrParam(urlMatch[1])
    } catch {
      return null
    }
  }

  if (trimmed.includes(ARCA_FE_QR_BASE_URL)) {
    const idx = trimmed.indexOf('?p=')
    if (idx >= 0) {
      const param = trimmed.slice(idx + 3).split(/[\s"'<>]/)[0]
      if (param) {
        try {
          return decodeArcaQrParam(param)
        } catch {
          return null
        }
      }
  ...
  if (/^[A-Za-z0-9_-]{20,}$/.test(trimmed)) {
    try {
      return decodeArcaQrParam(trimmed)
    } catch {
      return null
    }
  }
 ...
 */
export function extractArcaQrPayloadFromBuffer(buffer: Buffer): ArcaQrJsonPayload | null {
  const latin = buffer.toString('latin1')
  const match = latin.match(ARCA_QR_URL_RE)
  if (match?.[1]) {
    try {
      return decodeArcaQrParam(match[1])
    } catch {
      return null
    }
  }
  ...
  if (idx >= 0) {
    ...
    if (paramMatch?.[1]) {
      try {
        return decodeArcaQrParam(paramMatch[1])
      } catch {
        return null
      }
    }
 ...
 */
export function parseArcaQrFechaToIso(fecha: string): string | null {
 ...
 */
export function mapArcaQrToDocumentoCompraPreview(
  payload: ArcaQrJsonPayload,
  proveedorId: number | null,
): DocumentoCompraPreviewData {
  const tipo = arcaTipoComprobanteToLetter(payload.tipoCmp)
  ...
  const fechaIso = parseArcaQrFechaToIso(String(payload.fecha))
 ...
 */
export function arcaQrEmisorCuitDigits(payload: ArcaQrJsonPayload): string {
