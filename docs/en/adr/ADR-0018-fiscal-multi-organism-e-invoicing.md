...
4. **Capability-only stubs** for `uruguay_dgi` (`UruguayDgiFiscalAdapter`) and `mexico_sat_pac` (`MexicoSatFiscalAdapter`): `getCapabilities()` reports `implemented: false`; every operational method throws `FiscalAdapterNotImplementedError` instead of inventing SOAP/REST behavior that is **Not evidenced in current codebase**.
...
- **Follow-up:** a future ADR is required before removing `TenantFiscalConfig` (dual-read) or before implementing a live `uruguay_dgi` adapter. Mexico mock PAC is documented in [ADR-0024](ADR-0024-mexico-sat-cfdi-mock-pac.md); live commercial PAC remains Not evidenced.
