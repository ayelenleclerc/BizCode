...
| BP0-4 | P0 | Auth on domain API | Routes under `/api/clientes`, `/api/articulos`, `/api/rubros`, `/api/facturas`, `/api/formas-pago` use `requirePermission` when registered from [`server/registerRestDomainRoutes.ts`](../../../server/registerRestDomainRoutes.ts) (per-domain modules under `server/routes/`); app wiring in [`server/createApp.ts`](../../../server/createApp.ts); contract in [`docs/api/openapi.yaml`](../../api/openapi.yaml) |
| BP1-1 | P1 | Order (`pedido`) domain | **Commercial MVP (#132):** `Pedido`/`PedidoItem`, `/api/pedidos`, UI `/pedidos`; module gating `billing.orders` via #223. **Pending (#65):** full lifecycle `packed`…`collected` and order→logistics→collections wiring |
