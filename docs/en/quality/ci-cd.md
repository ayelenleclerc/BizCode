...
Optional env for a single tenant in dev/staging: `BIZCODE_TENANT_ID=<id>` (applies to `arca:retry-pending-job`, `arca:retry-pending`, `cobranzas:recordatorios`, `mercadopago:reconciliacion`, `shipping:tracking-refresh`, and `meli:token-refresh`). Optional `BIZCODE_RECORDATORIO_CANAL` (default `email`).
