...
**Entornos staging / producción (#152):** `.github/workflows/staging.yml` en push a `develop` publica tags GHCR `staging` / `sha-…` y despliega por SSH solo si existen `STAGING_DEPLOY_*` (`environment: staging`). El deploy SSH de producción en `deploy.yml` usa Environment **`production`**. Véase [entornos-despliegue.md](entornos-despliegue.md) y [ENVIRONMENTS.md](../../ENVIRONMENTS.md).
...
## Escaneo de dependencias e imágenes (#219)

- **Dependabot:** [`.github/dependabot.yml`](../../../.github/dependabot.yml) (npm + GitHub Actions semanal, grupos patch).
- **pnpm audit:** bloqueante HIGH+ en Quality Gate ([ADR-0017](../adr/ADR-0017-escaneo-dependencias.md)).
- **Snyk:** [`.github/workflows/snyk.yml`](../../../.github/workflows/snyk.yml) — requiere `SNYK_TOKEN`.
- **OWASP ZAP baseline (#194):** [`.github/workflows/zap.yml`](../../../.github/workflows/zap.yml) — API efímera o variable `ZAP_TARGET_URL` (staging/no prod); artefacto `zap-baseline-report`; no falla por WARN mientras exista `-I` (ver [pruebas-de-penetracion.md](pruebas-de-penetracion.md)).
- **Trivy imágenes:** tras build Docker en [`.github/workflows/deploy.yml`](../../../.github/workflows/deploy.yml), falla en CRITICAL antes de GHCR.
- Triage: [escaneo de dependencias y triage](escaneo-dependencias-y-triage.md).
...
- [x] Paso `pnpm audit --audit-level=high` tras install (**bloqueante** HIGH+; #219 / ADR-0017)
...
| `0 */2 * * *` | `npm run shipping:tracking-refresh` | Actualiza tracking de envíos en tránsito (`ShippingTrackingService`) para tenants con `ShippingCarrierConfig` activo; notifica managers al pasar a entregado (#193). |
| `0 */5 * * *` | `npm run meli:token-refresh` | Renueva access tokens OAuth de Mercado Libre (~6 h de vigencia) para tenants con `MeliConfig` activo próximo a vencer (#183). |
| `*/5 * * * *` | `npm run meli:catalog-sync` | Reintenta publicaciones de catálogo Mercado Libre (`MeliPublicacion`) en pending/error tras cambios de artículo (#184). |
| `0 * * * *` (horario) | `npm run meli:stock-reconcile` | Compara `available_quantity` de ML vs `Articulo.stock` en BizCode para publicaciones vinculadas; empuja la qty de BizCode a ML si difieren (sin duplicar `StockAjuste`) (#185). |
| `* * * * *` (cada minuto) | `npm run ecommerce:sync-worker` | Procesa `EcommerceSyncJob` vencidos (motor compartido): retry/backoff, SyncLog, alerta DLQ a super_admin tras 3 fallos (#189). |
...
| `0 2 * * *` (UTC) | `npm run backup:postgres` y luego `npm run backup:postgres:prune` | Backup PostgreSQL cifrado a `BIZCODE_BACKUP_DIR` (+ S3 CLI opcional). Requiere `BACKUP_ENCRYPTION_KEY`. Ver [backup-y-restauracion.md](backup-y-restauracion.md). |

Variable opcional para un solo tenant en dev/staging: `BIZCODE_TENANT_ID=<id>` (aplica a `arca:retry-pending-job`, `arca:retry-pending`, `cobranzas:recordatorios`, `mercadopago:reconciliacion`, `shipping:tracking-refresh`, `meli:token-refresh`, `meli:catalog-sync` y `meli:stock-reconcile`). Opcional `BIZCODE_ECOMMERCE_SYNC_LIMIT` para el tamaño de lote de `ecommerce:sync-worker` (por defecto 50). Opcional `BIZCODE_RECORDATORIO_CANAL` (por defecto `email`).
