...
| BP0-4 | P0 | Auth en API de dominio | Rutas bajo `/api/clientes`, `/api/articulos`, `/api/rubros`, `/api/facturas`, `/api/formas-pago` usan `requirePermission` al registrarse desde [`server/registerRestDomainRoutes.ts`](../../../server/registerRestDomainRoutes.ts) (módulos en `server/routes/`); cableado de la app en [`server/createApp.ts`](../../../server/createApp.ts); contrato en [`docs/api/openapi.yaml`](../../api/openapi.yaml) |
| BP1-1 | P1 | Dominio pedido (`pedido`) | **MVP comercial (#132):** `Pedido`/`PedidoItem`, `/api/pedidos`, UI `/pedidos`; gating modular `billing.orders` vía #223. **Pendiente (#65):** ciclo completo `packed`…`collected` y vínculo pedido→logística→cobranza |
