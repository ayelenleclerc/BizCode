...
4. **Contenedores:** subir tags `FROM` en Dockerfiles si CRITICAL está en la imagen base; rebuild y re-scan. Frontend usa `nginx:1.30.3-alpine` más `apk upgrade`. El runtime de API elimina `npm` (Node / `semantic-release`) y **no** instala Corepack/pnpm (`tar` vulnerable); arranca con `tsx` desde `node_modules/.bin`. Forzar `esbuild` ≥ 0.28.1 vía `pnpm.overrides` (CVE-2025-68121) con Vite `supported.destructuring`.
