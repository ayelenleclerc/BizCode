...
**Sugerencias modo check (#254):** `GET /api/clientes/:id/sugerencias-pedido` devuelve habituales (ranking por frecuencia a 6 meses, qty sugerida, chip de anomalía) y `ArticuloOferta` activas que no estén ya en habituales. En `/pedidos/nuevo` sin búsqueda, las secciones aparecen sobre el catálogo; el primer add usa la qty sugerida; los topes de stock reusan #256. El hydrate offline cachea sugerencias en SQLite. Escáner queda en #255.
