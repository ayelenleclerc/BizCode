...
3. **Snyk:** Job de Actions con secreto `SNYK_TOKEN`. `snyk test --severity-threshold=high --fail-on=upgradable` en PR/push; `snyk monitor` en push a `develop`/`main`. Sin token, el job falla con mensaje accionable.
...
6. **Fuera de alcance v1:** Socket.dev, Renovate, `dependency-review-action`, hardening mobile (#220).

## Consecuencias

- **Positivo:** PRs de deps automatizados; detección multicapa; evidencia ISO de vulnerabilidades técnicas.
- **Negativo:** Requiere `SNYK_TOKEN`; primeras corridas pueden fallar hasta remediar HIGH/CRITICAL con fix o subir imágenes base.
