...
4. **Mocks de homologación** para `uruguay_dgi` ([ADR-0025](ADR-0025-uruguay-dgi-cfe-mock.md)) y `mexico_sat_pac` ([ADR-0024](ADR-0024-mexico-sat-cfdi-mock-pac.md)): `getCapabilities()` reporta `implemented: true`; DGI/PAC live SOAP/REST sigue **no evidenciado**. Chile `chile_sii` permanece stub de solo capacidades (`FiscalAdapterNotImplementedError`).
...
- **Seguimiento:** se requiere un ADR futuro antes de eliminar `TenantFiscalConfig` (lectura dual) o antes de implementar un cliente live de `uruguay_dgi` / `chile_sii`. Mock CFE Uruguay en [ADR-0025](ADR-0025-uruguay-dgi-cfe-mock.md); mock PAC México en [ADR-0024](ADR-0024-mexico-sat-cfdi-mock-pac.md); PAC / DGI / SII live siguen sin evidenciarse.
