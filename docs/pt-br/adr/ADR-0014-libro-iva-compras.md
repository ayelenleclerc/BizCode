...
2. **Entrada de dados:** `POST /api/comprobantes-compra` (módulo `finance.ledger`, permissão `reports.financial.read`).
...
- **Negativo:** Registro manual de comprovantes até ampliar captura na UI.
