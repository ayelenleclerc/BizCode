...
4. **Stubs apenas de capacidades** para `uruguay_dgi` (`UruguayDgiFiscalAdapter`) e `mexico_sat_pac` (`MexicoSatFiscalAdapter`): `getCapabilities()` reporta `implemented: false`; todo método operacional lança `FiscalAdapterNotImplementedError` em vez de inventar comportamento SOAP/REST **não evidenciado no código atual**.
...
- **Acompanhamento:** um ADR futuro é necessário antes de remover `TenantFiscalConfig` (leitura dual) ou antes de implementar um adapter live de `uruguay_dgi`. O mock PAC do México está em [ADR-0024](ADR-0024-mexico-sat-cfdi-mock-pac.md); PAC comercial live permanece não evidenciado.
