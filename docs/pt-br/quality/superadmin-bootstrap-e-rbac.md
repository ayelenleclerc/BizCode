...
- **Seed do banco:** [`prisma/seed.ts`](../../../prisma/seed.ts) — upsert idempotente do tenant slug `platform` e usuário `ayelen` com papel `super_admin`. **`BIZCODE_SEED_SUPERADMIN_PASSWORD` é obrigatória** no `.env` antes de rodar o seed (mínimo 8 caracteres); o [`.env.example`](../../../.env.example) declara a variável sem valor padrão versionado.
