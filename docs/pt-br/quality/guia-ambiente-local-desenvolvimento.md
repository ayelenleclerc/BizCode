...
**Sugestões modo check (#254):** `GET /api/clientes/:id/sugerencias-pedido` devolve habituais (ranking por frequência em 6 meses, qty sugerida, chip de anomalia) e `ArticuloOferta` ativas que ainda não estão nos habituais. Em `/pedidos/nuevo` sem busca, as seções aparecem acima do catálogo; o primeiro add usa a qty sugerida; os tetos de estoque reutilizam #256. O hydrate offline cacheia sugestões no SQLite. Scanner fica em #255.
