...
Variável opcional para um único tenant em dev/staging: `BIZCODE_TENANT_ID=<id>` (vale para `arca:retry-pending-job`, `arca:retry-pending`, `cobranzas:recordatorios`, `mercadopago:reconciliacion` e `shipping:tracking-refresh`). Opcional `BIZCODE_RECORDATORIO_CANAL` (padrão `email`).
