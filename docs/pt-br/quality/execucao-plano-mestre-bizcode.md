...
| BP1-1 | P1 | Domínio pedido (`pedido`) | **MVP comercial (#132):** `Pedido`/`PedidoItem`, `/api/pedidos`, UI `/pedidos`; gating modular `billing.orders` via #223. **Pendente (#65):** ciclo completo `packed`…`collected` e vínculo pedido→logística→cobrança |
