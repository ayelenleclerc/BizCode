...
| Snyk `test` / `monitor` | `.github/workflows/snyk.yml` | **Bloqueia** HIGH+ com fix (`--fail-on=upgradable`); requer `SNYK_TOKEN` |
...
4. Sem o segredo, o workflow Snyk falha com mensagem acionável (de propósito).
...
4. **Containers:** subir tags `FROM` nos Dockerfiles se CRITICAL estiver na imagem base; rebuild e re-scan.
