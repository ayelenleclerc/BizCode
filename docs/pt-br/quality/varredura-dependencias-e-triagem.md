...
4. **Containers:** subir tags `FROM` nos Dockerfiles se CRITICAL estiver na imagem base; rebuild e re-scan. Frontend usa `nginx:1.30.3-alpine` mais `apk upgrade`. O runtime da API remove `npm` (Node / `semantic-release`) e **não** instala Corepack/pnpm (`tar` vulnerável); sobe com `tsx` de `node_modules/.bin`. Forçar `esbuild` ≥ 0.28.1 via `pnpm.overrides` (CVE-2025-68121) com Vite `supported.destructuring`.
