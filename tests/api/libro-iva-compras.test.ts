...
const MODULES =
  'core.auth,billing.credit_notes,billing.arca_cae,finance.ledger,fiscal.libro_iva,billing.orders,logistics.dispatches,logistics.picking,logistics.gps'
...
describe('GET /api/contabilidad/libro-iva-compras', () => {
  ...
  it('returns 403 when fiscal.libro_iva is disabled', async () => {
    vi.stubEnv('BIZCODE_TEST_MODULES', 'core.auth,billing.arca_cae,finance.ledger')
    const prisma = buildPrismaMock()
    const app = createApp(prisma)
    await request(app)
      .get('/api/contabilidad/libro-iva-compras')
